...
def cadastrar_view(request):
    if request.method == 'GET':
        ...
        login(request, user)
        return redirect('/nossos-cursos/<int:id>')
    else:
...
def cursos_escolhido_view(request, id):
    cursos = {
    ...
    curso = cursos[id]
    return render(request, "curso-escolhido.html", {"titulo": curso["titulo"], "descricao": curso["descricao"]})
